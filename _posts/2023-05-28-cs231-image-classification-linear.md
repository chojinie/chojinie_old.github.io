...
<p>
수식으로 표현해보겠습니다. $$ \math{i} $$ 번째 이미지의 전체 픽셀을 하나의 열 벡터로 만든 $$ \math{x_i} $$ 와 해당 미지의 lsbel $$ y_i $$ 가 있을 경우, score function(f)는 $$ x_i $$를 입력받아 클래스 점수를 나타내는 벡터 $$ s=f(x_i, W) $$를 출력합니다.(s는 점수 score를 의미) $$ x_i $$ 의 $$ j $$ 번째 클래스의 점수는 $$ s_j $$의 $$ j $$번째 요소인 $$ s_j = f(x_i, W)_{j} $$라 표현할 수 있다.
</p>
