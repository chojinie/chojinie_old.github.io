...
\begin{equation} L_i = -log(\frac{e^{f_{y_i}}}{\sum_{j}e^{f_j}}) L_i = -f_{y_i}+ log \sum_{j}e^{f_j}}) \end{equation}
...
개별 점수의 세부 사항에는 관심이 없습니다. 예를 들어, 점수가 [10, -100, -100]이거나 [10, 9, 9]라면, SVM은 여유 간격이 1로 만족되므로 손실이 0이기 때문에 차이를 느끼지 않습니다. 그러나 이러한 
